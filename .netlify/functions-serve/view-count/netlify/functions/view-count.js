...
async function handler(event) {
  ...
  if (event.httpMethod === "GET") {
    try {
      const response = await (0, import_node_fetch.default)(`${UPSTASH_REDIS_REST_URL}/get/${VIEW_COUNT_KEY}`, { headers });
    ...
    try {
      const response = await (0, import_node_fetch.default)(`${UPSTASH_REDIS_REST_URL}/incr/${VIEW_COUNT_KEY}`, {
        method: "POST",
        headers
      });
      const data = await response.json();
